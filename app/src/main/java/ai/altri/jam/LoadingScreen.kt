...
private class MissingAccessTokenException : Exception("Please try again after sign in")

private class UnauthorizedAccessException :
    Exception("Access denied. Please try again and grant the necessary permissions.")

private class MissingUrlException(message: String) : Exception(message)
...
internal fun LoadingRoute(
    onModelLoaded: () -> Unit = { }, onGoBack: () -> Unit = {}
) {
        ...
        job = launch(Dispatchers.IO) {
            try {
                if (!InferenceModel.modelExists(context)) {
                    // If using LOCAL_MODEL with a bundled model in assets, we don't need to download
                    if (BuildConfig.USE_LOCAL_MODEL && BuildConfig.BUNDLED_MODEL_ASSET_NAME.isNotEmpty()) {
                        Log.d(
                            TAG,
                            "Using bundled model from assets: ${BuildConfig.BUNDLED_MODEL_ASSET_NAME}"
                        )
                        // The model will be extracted in InferenceModel.modelPath()
                    }
                    // For regular models, download from URL
                    else if (!InferenceModel.model.url.isEmpty()) {
                        isDownloading = true
                        downloadModel(context, InferenceModel.model, client) { newProgress ->
                            progress = newProgress
                        }
                    }
                    // No URL and no bundled model - show error
                    else {
                        throw MissingUrlException("Model not available. Please select a different model.")
                    }
                }
...
private fun downloadModel(
    context: Context, model: Model, client: OkHttpClient, onProgressUpdate: (Int) -> Unit
) {
